...
public class KnuthDurstenfeldShuffle {
    ...
    /**
     * 算法步骤为：<p>
     * 1. 建立一个数组大小为 n 的数组 arr，分别存放 1 到 n 的数值；<p>
     * 2. 生成一个从 0 到 n - 1 的随机数 x；<p>
     * 3. 输出 arr 下标为 x 的数值，即为第一个随机数；<p>
     * 4. 将 arr 的尾元素和下标为 x 的元素互换；<p>
     * 5. 同2，生成一个从 0 到 n - 2 的随机数 x；<p>
     * 6. 输出 arr 下标为 x 的数值，为第二个随机数；<p>
     * 7. 将 arr 的倒数第二个元素和下标为 x 的元素互换；<p>
     * ……<p>
     * 如上，直到输出 m 个数为止<p>
     * <p>
